...
"""
 ...
 Note: similar to uvmcmcfit.py, but here we only save the uncorrelated samples.
...
try:
    ...
    raise("Please install plotutils...")
...
# keep uncorrelated samples only by sucessively thinning the chain when it has too many correlated smaples
nsam = 16000
nind = nsam/nwalkers
...
try:
    ...
    print("WARNING: cannot load saved state")
    ...
    runner = pr.EnsembleSamplerRunner(sampler, pzero) 
    runner.run_to_neff(nind, savedir='.')    # until there's at least nind independent samples for each walker
    # can then re-load the state from that directory to continue an interrupted run.


pu.plot_emcee_chains(runner.sampler.chain)
plt.savefig('runner_chain')
