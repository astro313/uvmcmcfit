...
(Deprecated, features merged w/ uvmcmcfit2.py, which will be implemented into uvmcmcfit.py at some point)
...
if not realpdf:
    ...
    print("*** Time to run {:d} steps of burn in: {:f}".format(burnin, time.time()-currenttime))
...
niter = 50 # 3500
saveint = 10 # 1000
nrun = int(numpy.round(niter/saveint))
totalsamples = nwalkers * niter
valid = {"yes": True, "y": True, "ye": True,
         "no": False, "n": False}
nsessions = 2

for r in range(nrun):
    if r == round(nrun/nsessions):
        email_self()
        ret = nonBlockingRawInput("Shall we continuue? (Y/N)", timeout=3600).lower()
        if ret in valid:
            if not valid[ret]:
                import sys
                sys.exit("Quiting after nrun of {:d}, with {} samples... ".format(r, sampler.flatlnprobability.shape[0]))
        else:
            sys.stdout.write("Please respond with 'yes' or 'no' "
                             "(or 'y' or 'n').\n")

    t1 = time.time()
    pos, prob, state, amp = sampler.run_mcmc(pos0, saveint)
    t2 = time.time()
...
#            "Steps:   {:d}".format(steps),
            ...
            "Mean lnprob and Max lnprob values: {:.4f} {:.4f}".format(numpy.mean(prob), numpy.max(prob)),
            "Time to run previous set of walkers (seconds): {:.1f}".format(t2 - t1),
            "Number of samples obtained: {:d}".format(sampler.flatlnprobability.shape[0])
            ]
    print('\n'.join(result))
